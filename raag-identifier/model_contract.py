...
"""Does this model version keep the pitch pathway convention?
    ...
    python3 model_contract.py best-model-09-01 --quick     # skip anything that runs CREPE
...
TONE_HZ = 220.0
TONE_SECONDS = 3.0
CENTS_TOLERANCE = 35.0      # generous: CREPE dithers its bins by +-20 cents on purpose
...
def tone(hz=TONE_HZ, seconds=TONE_SECONDS, sr=SR):
...
class Checks:
    ...
    def check_pitch_module_exists(self):
        if not hasattr(self.pkg, "pitch"):
            return (f"{self.name}.pitch is missing — CREPE has to be its own module, not "
                    f"buried in the melody branch")
        return None
    ...
    def check_pitch_api(self):
        ...
        for const in ("SR", "HOP", "CONFIDENCE"):
    ...
    # -- behaviour, needs CREPE -----------------------------------------------------

    def track_once(self):
        if self._track is None:
            self._track = self.pkg.pitch.track(tone())
        return self._track
    ...
    def check_track_is_reproducible(self):
        """Same audio, same answer — the dither seed doing its job."""
        a, b = self.pkg.pitch.track(tone()), self.pkg.pitch.track(tone())
        if not np.array_equal(np.asarray(a.f0_hz), np.asarray(b.f0_hz)):
            return ("two tracks of the same audio differ — torchcrepe's dither is drawing "
    ...
    def check_standalone_use(self):
        ...
        t = self.pkg.pitch.from_audio(tone(seconds=2.0), SR)
