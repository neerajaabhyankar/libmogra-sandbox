...
**Every model version here exposes its CREPE pitch track as a standalone module.** The
...
app computes it once and shares it. A model that buries CREPE inside its melody branch
forces every one of those to run CREPE again, and to guess at the settings.
...
| `pitch.track(y16000, ...)` | audio at `pitch.SR` -> a `PitchTrack` |
...
3. **A pitch track is reproducible.** torchcrepe dithers its bins from numpy's *global*
   RNG, so the seed is fixed and the caller's RNG state is saved and restored. Same
   recording, same answer, every process.
