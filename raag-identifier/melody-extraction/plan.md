...
Three monophonic pitch trackers, picked for being genuinely different algorithms (deep
...
## Shared pipeline (one implementation, three frame-level inputs)
...
- No Melodia/Essentia for now (install friction on macOS arm64) — revisit only if the
  three current methods prove insufficient.
