# Stage 6b (50-class): same as Stage 5b (08_finetune_warmstart_lastlayer.py) --
# warm-started fine-tune of only res_blocks[9] + head -- but with a longer
# training budget (more epochs, more patience).
#
# Stage 5b was still improving on val_loss when it early-stopped at epoch 38
# (best epoch=28, patience=10). This stage just doubles EPOCHS (40->80) and
# patience (10->20) to see whether val performance keeps improving with more
# time, or whether it plateaus/overfits past where 5b stopped. Everything else
# (split, crop strategy, model surface, LRs, eval methodology) is identical to
# Stage 5b.
#
...
# goes to outputs/full50_warm_lastlayer_longer_finetuned.
...
OUT_DIR = Path(__file__).parent / "outputs" / "full50_warm_lastlayer_longer_finetuned"
...
def main():
    ...
    plot_curves(history, best_epoch,
                "Stage 6b (50 classes, warm-started last-layer fine-tune, longer budget): training curves",
    ...
    eval_test_confusion_matrix(
        ...
        "Stage 6b (50 classes, warm-started last-layer fine-tune, longer budget) test confusion matrix",
