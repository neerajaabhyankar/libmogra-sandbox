# Stage 6a (50-class): warm-started fine-tune of the LAST TWO backbone residual
# blocks (res_blocks[8:]) + head.
#
# Follow-up to Stage 5b (08_finetune_warmstart_lastlayer.py), which unfroze only
# res_blocks[9] and was the best result so far (test acc 0.163, macro-F1 0.132).
# This stage gives the model a slightly bigger trainable surface --
# res_blocks[8] and res_blocks[9] + head -- while keeping conv_first and
# res_blocks[0..7] frozen (eval mode, requires_grad=False, BN running stats
# untouched). Head + feature scaler warm-started from Stage 3
# (outputs/full50/head_checkpoint.pt, same as 07/08).
#
# Train/val split, crop strategy, and eval methodology match Stage 5b.
#
...
# goes to outputs/full50_warm_last2blocks_finetuned.
...
OUT_DIR = Path(__file__).parent / "outputs" / "full50_warm_last2blocks_finetuned"
...
def set_mode(model, train):
    ...
    if train:
        ...
        for i in range(len(model.res_blocks) - N_UNFROZEN_BLOCKS):
...
def main():
    ...
    # Freeze everything except the last two residual blocks + head.
    for p in model.conv_first.parameters():
        p.requires_grad = False
    for block in model.res_blocks[:-N_UNFROZEN_BLOCKS]:
        for p in block.parameters():
            p.requires_grad = False

    last_blocks_params = [p for block in model.res_blocks[-N_UNFROZEN_BLOCKS:] for p in block.parameters()]
    head_params = list(model.head.parameters())
    n_trainable = sum(p.numel() for p in last_blocks_params + head_params)
    n_total = sum(p.numel() for p in model.parameters())
    print(f"Trainable params: {n_trainable:,} / {n_total:,} (last {N_UNFROZEN_BLOCKS} res_blocks + head)")

    opt = torch.optim.Adam([
        {"params": last_blocks_params, "lr": LAST_BLOCKS_LR, "weight_decay": LAST_BLOCKS_WD},
    ...
    plot_curves(history, best_epoch,
                "Stage 6a (50 classes, warm-started last-2-blocks fine-tune): training curves",
    ...
    torch.save({
        "config": dict(hidden_dims=HIDDEN_DIMS, batchnorm=BATCHNORM, dropout=DROPOUT,
                        last_blocks_lr=LAST_BLOCKS_LR, last_blocks_wd=LAST_BLOCKS_WD,
                        head_lr=HEAD_LR, head_wd=HEAD_WD, fixed_length=FIXED_LENGTH,
                        batch_size=BATCH_SIZE, n_unfrozen_blocks=N_UNFROZEN_BLOCKS,
    ...
    eval_test_confusion_matrix(
        ...
        "Stage 6a (50 classes, warm-started last-2-blocks fine-tune) test confusion matrix",
