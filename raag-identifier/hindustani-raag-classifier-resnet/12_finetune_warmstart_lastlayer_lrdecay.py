# Stage 6c (50-class): same as Stage 5b (08_finetune_warmstart_lastlayer.py) --
# warm-started fine-tune of only res_blocks[9] + head -- but with a cosine LR
# decay schedule on both param groups.
#
# Stage 5b's val loss/acc oscillated epoch-to-epoch (while trending down/up
# overall) even though train loss decreased smoothly. A decaying LR should
# shrink the step size as training progresses, tamping down that oscillation
# in the later epochs without changing the trainable surface (res_blocks[9] +
# head) or the warm start. Same EPOCHS/PATIENCE budget as Stage 5b so the
# schedule -- not extra training time -- is the only new variable; CosineAnnealingLR
# with T_max=EPOCHS decays each param group's LR from its Stage-5b value down to
# ~1/10th of it over the full run.
#
# Does not touch outputs/probe, outputs/sweep, outputs/model, outputs/embeddings_all,
# outputs/full50, outputs/full50_finetuned, outputs/full50_warm_finetuned, or
# outputs/full50_warm_lastlayer_finetuned (Stages 0-5b artifacts). Everything new
# goes to outputs/full50_warm_lastlayer_lrdecay_finetuned.
...
OUT_DIR = Path(__file__).parent / "outputs" / "full50_warm_lastlayer_lrdecay_finetuned"
...
# Cosine-decay each param group's LR from its initial value down to 1/10th
# of it over EPOCHS epochs.
LR_ETA_MIN_FRAC = 0.1
...
def main():
    ...
    # Cosine-decay each param group from its own initial LR to
    # LR_ETA_MIN_FRAC of it, applied manually (per-group eta_min isn't
    # supported by torch.optim.lr_scheduler.CosineAnnealingLR).
    ...
    for epoch in range(EPOCHS):
        ...
        history["lrs"].append([g["lr"] for g in opt.param_groups])
        ...
        # Cosine-decay each group from its own base LR to LR_ETA_MIN_FRAC * base LR.
        progress = (epoch + 1) / EPOCHS
        cosine_frac = LR_ETA_MIN_FRAC + (1 - LR_ETA_MIN_FRAC) * 0.5 * (1 + np.cos(np.pi * progress))
        for g, base_lr in zip(opt.param_groups, base_lrs):
            g["lr"] = base_lr * cosine_frac
    ...
    plot_curves(history, best_epoch,
                "Stage 6c (50 classes, warm-started last-layer fine-tune, cosine LR decay): training curves",
    ...
    torch.save({
        "config": dict(hidden_dims=HIDDEN_DIMS, batchnorm=BATCHNORM, dropout=DROPOUT,
                        ...
                        batch_size=BATCH_SIZE, lr_eta_min_frac=LR_ETA_MIN_FRAC,
                        warm_start_head=str(STAGE3_HEAD_CKPT)),
    ...
    eval_test_confusion_matrix(
        ...
        "Stage 6c (50 classes, warm-started last-layer fine-tune, cosine LR decay) test confusion matrix",
